...
for (( year=${START_YEAR}; year<=${END_YEAR}; year++ )); do
    ...
    for month in {01..12}; do
        ...
        if [ "${DATASET_TYPE}" = "monthly" ]; then
            ...
            # IMPORTANT: shift time by 1 hour to get correct diurnal mean
            # ERA5 measures 0:00 for the interval 23:00-0:00, 
            # so shifting by 1 hour gets it to the same interval as IMERG 00:00-01:00 etc.

            cdo -O shifttime,1hour "${monthly_input}" "${monthly_input}_shifted.nc"
            cdo -O dhourmean "${monthly_input}_shifted.nc" "${outfile}"
            rm -f "${monthly_input}_shifted.nc"
            ...
            #cdo -O shifttime,0.5hour "${monthly_input}" "${monthly_input}_shifted.nc"
