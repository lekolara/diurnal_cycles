...
INPUT_FILE = "/scratch/leko/IMERG/IMERG_1_deg_diurnal/IMERG_diurnal_climatology_2018_2023.nc"

VAR_NAME = "pr"          # precipitation variable
RELATIVE = False        # True -> normalize by monthly mean
COARSEN = None          # e.g. 2 -> 2x2 deg spatial averaging

if RELATIVE:
    OUTPUT_FILE_AMPLITUDE = f"/scratch/leko/IMERG/IMERG_1_deg_diurnal/IMERG_diurnal_amplitude_{RELATIVE}deg.nc"
else:
    OUTPUT_FILE_AMPLITUDE = f"/scratch/leko/IMERG/IMERG_1_deg_diurnal/IMERG_diurnal_amplitude.nc"

OUTPUT_FILE_PHASE = f"/scratch/leko/IMERG/IMERG_1_deg_diurnal/IMERG_diurnal_phase.nc"
...
for m in da.month.values:
    ...
    if COARSEN:
        y = y.coarsen(lat=COARSEN, lon=COARSEN).mean()
